...
export class PreloadSink implements ILogSink {
    public static SerializeRaw: Record<string, boolean> =
        {
            ['nil']: false,
            ['boolean']: true,
            ['number']: true,
    ...
    public LogLevel(): LogLevel { return this.logLevel; }
    ...
    public Log(level: LogLevel, events: LogEvent[]): void {
        let json: string = '{';
