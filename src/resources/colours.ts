// STANDARD COLOURS
export const COL_PURPLE = `|cff6f2583`;
export const COL_TEAL = `|cff00ffff`;
export const COL_ORANGE = '|cffd45e19';
export const COL_GOLD = `|cffffcc00`;
export const COL_PINK = `|cffff8080`;
export const COL_RED = `|cffb52166`;
export const COL_LIGHT_RED = `|cffc81e1e`;
export const COL_GREY = `|cff627781`;

// CONTEXT COLOURS
export const COL_GOOD = `|cff18bc9c`;
export const COL_BAD = COL_RED;
export const COL_ATTATCH = COL_RED;
export const COL_INCOME = `|cfff9a602`;
export const COL_MISC = COL_GREY;
export const COL_INFO = `|cff00ffff`;
export const COL_RESOLVE = `|cffffff00`;
export const COL_ALIEN = `|cff8a6df2`;

// MECHANIC COLOURS
export const COL_FREEZE = COL_TEAL;

// FLOOR COLOURS
export const COL_FLOOR_1 = `|cfff5a742`;
export const COL_FLOOR_2 = `|cff41dee0`;
export const COL_CARGO_A = `|cff008080`;
export const COL_CARGO_B = `|cff7b778c`;
export const COL_BRIDGE = COL_GOLD;

export const COL_VENTS = `|cff666633`;

export const COL_GOD = '|cffffcc00';
export const COL_SYS = '|cffc81e1e';
export const COL_MISC_MESSAGE = '|cff00ffff';